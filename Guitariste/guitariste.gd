...
func _attaquer(target):
	...
	if target.has_node("Health"):
		...
		print("🎸 Guitariste attaque ", target.name, " (", attack_damage, " dégâts)")
		guitaristesound.play()
	
	# ← TIMER SANS AWAIT (fix create_timer)
	var tween = create_tween()
	tween.tween_callback(func(): can_attack = true).set_delay(attack_cooldown)
